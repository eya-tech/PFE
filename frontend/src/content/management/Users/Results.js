...
const applyFilters = (users, query, filters) => {
  return users.filter((user) => {
    ...
    if (query) {
      ...
      if (filters.role && user.role.titre !== filters.role) {
        matches = false;
      }
...
const Results = ({ users, setUsers }) => {
  ...
  const handleDeleteCompleted = async() => {
    ...
    let ids = selectedItems.map((id)=> users[id]._id )
    ...
    if (res.data.success) {
      ...
      console.log(users)
        ...
        enqueueSnackbar(t('something is wrong with the server'), {
        variant: 'error',
        anchorOrigin: {
          vertical: 'top',
          horizontal: 'right'
      ...
      {toggleView === 'table_view' && (
        <Card>
          <Box p={2}>
            ...
            {selectedBulkActions && <BulkActions setOpenConfirmDelete={setOpenConfirmDelete} />}
          </Box>
                    ...
                    {paginatedUsers.map((user) => {
                      ...
                      return (
                        <TableRow hover key={user.id} selected={isUserSelected}>
                          ...
                          <TableCell>{getUserRoleLabel(user.role.titre)}</TableCell>
            ...
            >
              {paginatedUsers.length !== 0 && (
                <>
                  ...
                  {selectedBulkActions && (
                    <Box flex={1} pl={2}>
                      <BulkActions setOpenConfirmDelete={setOpenConfirmDelete} />
                    </Box>
                  )}
                </>
              )}
                      ...
                      >
                        ...
                        >
                          <Box
                            ...
                            justifyContent="space-between"
                          >
                            {getUserRoleLabel(user.role.titre)}
                            <IconButton
                              color="primary"
                              sx={{
                                p: 0.5
